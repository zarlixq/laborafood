...
export default function AnalysesClient() {
  ...
  const filtered = useMemo(() => {
    ...
    return analyses.filter((a) => {
      ...
      const name = getAnalysisName(a, locale).toLowerCase();
      const desc = getAnalysisDescription(a, locale).toLowerCase();
      const okQ = !q || name.includes(q) || desc.includes(q) || a.method.toLowerCase().includes(q);
      return okCat && okQ;
    });
  }, [active, query, locale]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
      <aside className="lg:col-span-3 lg:sticky lg:top-32 self-start">
        ...
        <div className="flex flex-wrap lg:flex-col gap-2">
          <CategoryChip
            label={`${tCommon("all")} (${analyses.length})`}
            active={active === "all"}
      ...
      <div className="lg:col-span-9">
        <div className="text-sm text-ink-soft mb-4">
          {filtered.length} {tCommon("results")}
        </div>
        {filtered.length === 0 ? (
            ...
            {filtered.map((a) => (
              ...
              >
                ...
                <div className="flex flex-col md:items-end gap-1 flex-shrink-0">
                  ...
                  <span className="text-xs text-ink-soft">{getAnalysisDuration(a, locale)}</span>
                </div>
              </li>
            ))}
          </ul>
  ...
  return (
    <button
      ...
      className={`text-left text-sm rounded-full px-4 py-2 transition-colors ${
        active ? "bg-ink text-white" : "bg-white border border-line text-ink hover:border-brand hover:text-brand"
      }`}
    >
      {label}
    </button>
